...
def check_brain_age_predictor(
    ...
    if not isinstance(number_of_epochs, int) or number_of_epochs <= 0:
        raise ValueError("Please set 'number_of_epochs' to a positive "
                         "integer!")
    ...
    if architecture == 'sfcn':
        if not image_dimensions == (160, 192, 160):
            raise ValueError("Please set 'image_dimensions' to "
                             "(160, 192, 160) to use SFCN architecture!")
    ...
    if pretrained_weights is not None and not isinstance(
            pretrained_weights, str):
        raise ValueError("Please set 'pretrained_weights' to a string!")

    if pretrained_weights is not None and not exists(pretrained_weights):
